    ...
    <main className="min-h-screen bg-[#f0ede3] text-[#171713] selection:bg-[#74795a] selection:text-white">
      ...
      <section className="relative overflow-hidden border-b border-black/10">
        ...
        <div className="mx-auto grid min-h-[68vh] max-w-7xl items-center gap-12 px-6 py-20 lg:grid-cols-[1.25fr_.75fr] lg:px-10 lg:py-20">
          <div className="relative z-10">
            <p className="mb-6 text-[12px] font-bold uppercase tracking-[0.22em] text-[#686d4e] md:text-[11px] md:tracking-[0.3em]">{t.eyebrow}</p>
          ...
          <div className="relative hidden min-h-[460px] lg:block">
            ...
            {t.media.heroImageUrl ? (
              <div className="absolute inset-0 -rotate-3 overflow-hidden rounded-[3rem] border border-black/10 bg-[#74795a] shadow-xl">
                <img src={t.media.heroImageUrl} alt="" className="absolute inset-0 h-full w-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/55 via-black/5 to-transparent" />
                <img src={logoUrl} alt="" className={`absolute left-9 top-9 h-20 w-20 object-contain ${defaultLogo ? 'brightness-0 invert' : ''}`} />
                <div className="absolute bottom-9 left-9 right-9 border-t border-white/35 pt-5 text-white">
                  <p className="text-xs font-bold uppercase tracking-[0.25em] text-white/75">F.LLI FRANCHI · Digital studio</p>
                </div>
              </div>
            ) : (
      ...
      <section id="servicos" className="scroll-mt-24 border-b border-black/10 bg-[#171713] text-[#f0ede3]">
        <div className="mx-auto max-w-7xl px-6 py-24 lg:px-10 lg:py-32">
          <div className="grid gap-10 lg:grid-cols-2">
            <div>
              <p className="text-[12px] font-bold uppercase tracking-[0.22em] text-[#a9ad8d] md:text-[11px] md:tracking-[0.3em]">{t.servicesKicker}</p>
            ...
            {t.services.map(([number, title, text]) => (
              <article key={number} className="group border-b border-r border-white/10 p-8 transition hover:bg-white/[0.035] md:p-9 lg:min-h-72">
                <div className="flex items-start justify-between">
                  <span className="text-[12px] font-bold tracking-[0.18em] text-[#a9ad8d] md:text-xs md:tracking-[0.2em]">{number}</span>
                ...
                <p className="mt-6 max-w-md text-[15px] leading-7 text-white/45 md:text-sm md:leading-6">{text}</p>
              </article>
            ))}
          </div>
        </div>
            ...
            <div className="border-t border-black/15">
              {t.process.map(([number, title, text]) => (
                <article key={number} className="grid gap-6 border-b border-black/15 py-9 sm:grid-cols-[4rem_10rem_1fr] sm:items-start">
                  ...
                  <p className="text-[15px] leading-7 text-black/50 md:text-sm md:leading-6">{text}</p>
                </article>
              ))}
            </div>
          </div>
      ...
      <section id="contato" className="scroll-mt-24 bg-[#11110f] text-white">
        <div className="mx-auto grid max-w-7xl gap-14 px-6 py-24 lg:grid-cols-[.85fr_1.15fr] lg:px-10 lg:py-32">
          <div>
            <p className="text-[12px] font-bold uppercase tracking-[0.22em] text-[#a9ad8d] md:text-[11px] md:tracking-[0.3em]">{t.contactKicker}</p>
            <h2 className="mt-5 max-w-xl font-serif text-4xl leading-[0.95] tracking-[-0.04em] md:text-5xl">{t.contactTitle}</h2>
            <p className="mt-7 max-w-lg text-[15px] leading-7 text-white/45 md:text-sm md:leading-7">{t.contactText}</p>
            <div className="mt-10 flex items-center gap-3 text-[13px] uppercase tracking-[0.18em] text-white/45 md:text-xs">
          ...
          <form onSubmit={handleSubmit} className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-6 md:p-8">
            <div className="grid gap-5 md:grid-cols-2">
              <label className="text-[13px] font-semibold text-white/55 md:text-xs">{t.form.name}
                <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-[16px] text-white outline-none transition placeholder:text-white/20 focus:border-[#a9ad8d] md:text-sm" />
              </label>
              <label className="text-[13px] font-semibold text-white/55 md:text-xs">{t.form.email}
                <input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-[16px] text-white outline-none transition focus:border-[#a9ad8d] md:text-sm" />
              </label>
              <label className="text-[13px] font-semibold text-white/55 md:text-xs">{t.form.whatsapp}
                <input required type="tel" value={form.whatsapp} onChange={(e) => setForm({ ...form, whatsapp: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-[16px] text-white outline-none transition focus:border-[#a9ad8d] md:text-sm" />
              </label>
              <label className="text-[13px] font-semibold text-white/55 md:text-xs">{t.form.service}
                <select required value={form.service} onChange={(e) => setForm({ ...form, service: e.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-[#191916] px-4 py-3 text-[16px] text-white outline-none transition focus:border-[#a9ad8d] md:text-sm">
                  <option value="">{t.form.servicePlaceholder}</option>
                  {t.form.services.map((service) => <option key={service} value={service}>{service}</option>)}
                </select>
              </label>
            </div>
            <label className="mt-5 block text-[13px] font-semibold text-white/55 md:text-xs">{t.form.message}
              <textarea rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="mt-2 w-full resize-none rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-[16px] text-white outline-none transition focus:border-[#a9ad8d] md:text-sm" />
            </label>
            ...
            <div className="mt-5 flex items-center gap-2 text-[12px] uppercase tracking-[0.15em] text-white/25 md:text-[10px]"><Check className="h-3 w-3" /> Firebase workflow connected</div>
          </form>
        </div>
      </section>
      ...
      <footer className="bg-[#11110f] text-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-7 border-t border-white/10 px-6 py-9 sm:flex-row sm:items-center sm:justify-between lg:px-10">
          ...
          <p className="text-[12px] uppercase tracking-[0.2em] text-white/35 md:text-[10px]">{t.footer}</p>
          <Link to="/" className="text-[12px] font-bold uppercase tracking-[0.2em] text-[#a9ad8d] md:text-[10px]">Brasil / Italia</Link>
        </div>
      </footer>
    </main>
  );
